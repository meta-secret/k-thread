...
test.describe('design surfaces', () => {
  ...
  test('structure surface after a note exists', async ({ page }) => {
    await enterWorkspaceFromLanding(page)
    await openStructureView(page)
    await page.waitForTimeout(700)
    await expect(page.locator('.app.ready')).toHaveScreenshot('structure-shell.png', {
      animations: 'disabled',
      maxDiffPixelRatio: 0.05,
    })
  })

  test('links surface after a note exists', async ({ page }) => {
    await enterWorkspaceFromLanding(page)
    await openLinksView(page)
    await page.waitForTimeout(700)
    await expect(page.locator('.app.ready')).toHaveScreenshot('links-shell.png', {
      animations: 'disabled',
      maxDiffPixelRatio: 0.05,
    })
  })
  ...
  test('round-trip note → structure → links → note', async ({ page }) => {
    await enterWorkspaceFromLanding(page)
    await openStructureView(page)
    await openLinksView(page)
